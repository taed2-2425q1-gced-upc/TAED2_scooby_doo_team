...
def preapare_validation_dataloaders(
    input_valid_images_path: Path, batch_size: int
) -> data.DataLoader:
    """
    ...
    class_folders = [
        folder for folder in input_valid_images_path.iterdir() if folder.is_dir()
    ]
    ...
    valid_dataset = data.TensorDataset(
        torch.stack(validation_images), torch.tensor(validation_labels).long()
    )
