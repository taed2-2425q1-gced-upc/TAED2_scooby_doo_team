...
class SharpnessCheck(TrainTestCheck):
    """Check if the Sharpness Index of images in train and test datasets meets the minimum threshold."""
    ...
    def update(self, context: Context, batch: BatchWrapper, dataset_kind: DatasetKind):
        """Calculate Sharpness Index for each image in the batch and add it to the corresponding list."""
    ...
    def compute(self, context: Context) -> CheckResult:
        ...
        sharpness_averages = {kind: np.mean(values) if values else 0 for kind, values in self._sharpness_values.items()}
    ...
    def add_sharpness_condition(self, minimum_threshold: float):
        """Add a condition to check that all Sharpness Index values are above the minimum threshold."""
        def sharpness_condition(value: Dict[str, List[float]]) -> ConditionResult:
            failed_images = {k: [sharpness for sharpness in v if sharpness < minimum_threshold] for k, v in value["Sharpness values per dataset"].items()}
            if all(len(failed) <= 10 for failed in failed_images.values()):
                return ConditionResult(ConditionCategory.PASS, f"Most of the images have Sharpness Index above {minimum_threshold}.")
            else:
...
shatp_check = SharpnessCheck().add_sharpness_condition(SHARPNESS_MINIMUM_THRESHOLD)
custom_suite.add(shatp_check)
