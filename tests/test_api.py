"""Module for testing API functionalities in the ScoobyDoo project."""
...
from src.app.api import (
    ...
    allowed_file_format,
    image_to_tensor,
    save_rating_api_to_csv,
    rating_api,
    save_rating_models_to_csv,
    models_rate,
    rating_models_api,
    get_model_summary,
    ratings_data,
...
def test_load_ratings():
    ...
    """
    with open(rating_models_api, "r",encoding="utf-8") as f:
        assert json.load(f)
...
def test_allowed_file_format():
    """
    Test the allowed_file_format function
    """
    ...
    for img_format in image_formats:
        ...
        assert allowed_file_format(
            mock_file
        ), f"El formato {img_format} debería ser permitido."
    # Prueba con un formato no permitido
    mock_file_invalid = MagicMock(filename=f"{TEST_IMAGE_DIR}/test_image.txt")
    assert not allowed_file_format(
        mock_file_invalid
    ), "El formato txt no debería ser permitido."


def test_image_to_tensor():
    """
    Test the image_to_tensor function
    """
    ...
    assert tensor.shape == (
        ...
        224,
        224,
    ), "El tensor no tiene las dimensiones esperadas"
...
def test_rate_api(client, valid_rating):
    """
    Test the rate_api endpoint with valid ratings
    """
...
def test_rate_api_invalid_ratings(client, invalid_rating):
    """
    Test the rate_api endpoint with invalid ratings
    """
...
def test_rate_api_unexpected_exception(client):
    """
    Test the rate_api endpoint with an unexpected exception
    """
    with patch(
        "src.app.api.save_rating_api_to_csv", side_effect=Exception("Unexpected error")
    ):
...
def test_rate_model_valid_rate(client):
    """
    Test the rate_model endpoint with a valid rating
    """
    with patch("src.app.api.save_rating_models_to_csv") as mock_save:
        model_name = "Model_1"

        response = client.post("/models/rate/Model_1?rating=4")
        assert ratings_data[model_name]["average_rating"] == round(
            sum(ratings_data[model_name]["ratings"])
            / len(ratings_data[model_name]["ratings"]),
            2,
        )
...
def test_rate_model_invalid_rate(client):
    """
    Test the rate_model endpoint with an invalid rating
    """
...
def test_rate_model_invalid_model(client):
    """
    Test the rate_model endpoint with an invalid model name
    """
...
def test_get_models_list(client):
    """
    Test the get_models_list endpoint
    """
    mock_data = {
...
def test_model_stats_valid_model(client):
    """
    Test the model-summary endpoint with a valid model name
    """
...
def test_model_stats_invalid_model(client):
    """
    Test the model-summary endpoint with an invalid model name
    """
    model_name = "Model_9"
    response = client.get(f"/model-summary?Model_name={model_name}")
    assert response.status_code == HTTPStatus.BAD_REQUEST
...
def test_model_stats_bad_model_name_format(client):
    """
    Test the model-summary endpoint with an invalid model name format
    """
...
def test_model_stats_summary_exception(client):
    """
    Test the model-summary endpoint with an exception during summary generation
    """
    with patch(
        "src.app.api.get_model_summary",
        side_effect=Exception("Summary generation error"),
    ):
        with patch("src.app.api.model_list", mock_model_list):
            model_name = "Model_1"
            response = client.get(f"/model-summary?Model_name={model_name}")
...
def test_health_check_negative_index(client):
    ...
    with patch("src.app.api.model_list", mock_model_list):
        ...
        assert (
            response.json()["detail"]
            == "Invalid model name format. Ensure it follows 'Model_<number>'"
        )
...
def test_predict_image_single_file(client):
    """
    Test the predict endpoint with a single image file
    """
    model_name = "Model_1"
...
def create_fake_jpeg_image():
    """
    Test utility function to create a fake JPEG image
    """
    image = io.BytesIO()
...
def test_predict_image_zip(client):
    """
    Test the predict endpoint with a zip file containing an image
    """
    model_name = "Model_1"
    ...
    with patch("src.app.api.zipfile.ZipFile", new_callable=MagicMock) as mock_zip:
        ...
        mock_zip_instance.read.side_effect = lambda name: (
            file_content.getvalue() if name == "test_image.jpg" else None
        )
...
def test_predict_image_invalid_file(client):
    """
    Test the predict endpoint with an invalid image file
    """
...
def test_predict_image_invalid_format(client):
    ...
    response = client.post(
        "/predict/Model_1",
        files={"files": [("test_image.txt", b"not an image", "text/plain")]},
    )
    assert response.status_code == HTTPStatus.BAD_REQUEST
...
def test_predict_image_gif_invalid_file(client):
    """
    Test the predict endpoint with an invalid GIF file
    """
...
def test_predict_image_gif_file(client):
    """
    Test the predict endpoint with a valid GIF file
    """
    model_name = "Model_1"
...
def test_predict_image_png_invalid_file(client):
    """
    Test the predict endpoint with an invalid PNG file
    """
...
def test_predict_image_png_file(client):
    """
    Test the predict endpoint with a valid PNG file
    """
    model_name = "Model_1"
...
def test_predict_image_webp_invalid_file(client):
    """
    Test the predict endpoint with an invalid WebP file
    """
...
def test_predict_image_webp_file(client):
    """
    Test the predict endpoint with a valid WebP file
    """
    model_name = "Model_1"
...
def delete_last_row(file_path):
    """
    Test utility function to delete the last row of a CSV file
    """
...
def test_save_rating_api_to_csv():
    """
    Test the save_rating_api_to_csv function
    """

    raiting = 5
    save_rating_api_to_csv(raiting)
    with open(rating_api, mode="r", newline="",encoding="utf-8") as file:
...
def test_save_rating_models_to_csv():
    """
    Test the save_rating_models_to_csv function
    """
    rating = 3
    modelname = "Model_1"
    save_rating_models_to_csv(modelname, rating)

    with open(models_rate, mode="r", newline="", encoding="utf-8") as file:
        csv_reader = csv.reader(file)
