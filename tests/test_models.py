...
from src.models.train import (
    combine_hyperparameters,
    prepare_training_objects,
    get_model,
    prepare_hyperparameters_combinations,
    get_optimizer,
    preapare_train_dataloaders,
    training,
)
from src.models.evaluate import (
    preapare_validation_dataloaders,
    load_image,
    evaluate_model,
)
...
def split_data_mock(loaded_data, mock_prepare_parameters):
    ...
    x_train, x_valid, x_test, y_train, y_valid, y_test = split_data(
        loaded_data, mock_prepare_parameters
    )
...
def cats_dogs_test_data():
    ...
    def obtain_test_data(input_test_images_path, batch_size):
        ...
        class_folders = [
            folder for folder in input_test_images_path.iterdir() if folder.is_dir()
        ]
        ...
        test_dataset = data.TensorDataset(
            torch.stack(test_images), torch.tensor(test_labels).long()
        )
...
def test_split_data(loaded_data, mock_prepare_parameters):
    ...
    expected_valid_size = int(
        expected_valid_test * mock_prepare_parameters["valid_size"]
    )
    expected_test_size = int(expected_valid_test - expected_valid_size)

    assert (
        len(x_train) == expected_train_size or len(x_train) == expected_train_size + 1
    ), f"Train set size is incorrect, expected {expected_train_size}, got {len(x_train)}"
    assert (
        len(x_valid) == expected_valid_size or len(x_valid) == expected_valid_size + 1
    ), f"Validation set size is incorrect, expected {expected_valid_size}, got {len(x_valid)}"
    assert (
        len(x_test) == expected_test_size or len(x_test) == expected_test_size + 1
    ), f"Test set size is incorrect, expected {expected_test_size}, got {len(x_test)}"
...
def test_save_data(split_data_mock):
    ...
    ).exists(), "File X_train.csv does not exist"
    assert (
        PROCESSED_DATA_DIR / "y_train.csv"
    ).exists(), "File y_train.csv does not exist"
    assert (
        PROCESSED_DATA_DIR / "X_valid.csv"
    ).exists(), "File X_valid.csv does not exist"
    assert (
        PROCESSED_DATA_DIR / "y_valid.csv"
    ).exists(), "File y_valid.csv does not exist"
    assert (
        PROCESSED_DATA_DIR / "X_test.csv"
    ).exists(), "File X_test.csv does not exist"
    assert (
        PROCESSED_DATA_DIR / "y_test.csv"
    ).exists(), "File y_test.csv does not exist"
...
def test_preapare_train_dataloaders():
    ...
    batch_size = 32
    train_dataloader = preapare_train_dataloaders(PROCESSED_TRAIN_IMAGES, batch_size)
...
def test_get_combine_hyperparameters():
    ...
    assert (
        len(combinations) == num_combinations
    ), "Combinations have not the expected length."
...
def test_prepare_hyperparameters_combinations():
    ...
    """
    parameters = {
        "hyperparameters": {"learning_rate": [0.01, 0.001], "batch_size": [16, 32]},
        "combinations": 4,
    }
...
def test_get_model():
    ...
    model = get_model(
        "VisualTransformer", "google/vit-base-patch16-224", ["cat", "dog"]
    )
...
def test_get_optimizer(best_model):
    ...
    ), "Optimizer has not the expected type"
...
def test_train(mock_train_parameters, best_model):
    ...
    optimizer = torch.optim.Adam(
        ...
        lr=mock_train_parameters["learning_rate"],
        weight_decay=mock_train_parameters["weight_decay"],
    )
    device = torch.device("cpu")
    loss_function = torch.nn.CrossEntropyLoss()

    _, loss_per_epoch = training(
        mock_train_parameters, best_model, optimizer, device, loss_function
    )

    # Verificar que se entrena y que la pérdida por epoch está decreciendo
    assert (
        len(loss_per_epoch) == mock_train_parameters["num_epochs"]
    ), "Epochs don't match"
...
def test_preapare_validation_dataloaders():
    ...
    valid_dataloader = preapare_validation_dataloaders(
        PROCESSED_VALID_IMAGES, batch_size
    )
